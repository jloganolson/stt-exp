...
def run_live(config: LiveConfig) -> None:
    ...
    if "deepgram" in config.providers:
        threads.append(
            threading.Thread(
                ...
                args=(config, queues["deepgram"], stop_event, emit),
                daemon=True,
            )
        )
    if "voxtral" in config.providers:
        threads.append(
            threading.Thread(
                ...
                args=(config, queues["voxtral"], stop_event, emit),
                daemon=True,
            )
        )
    if "sherpa" in config.providers:
        threads.append(
            threading.Thread(
                ...
                args=(config, queues["sherpa"], stop_event, emit),
                daemon=True,
            )
        )
    if "moonshine" in config.providers:
        threads.append(
            threading.Thread(
                ...
                args=(config, queues["moonshine"], stop_event, emit),
                daemon=True,
            )
        )
    if "parakeet" in config.providers:
        threads.append(
            threading.Thread(
                ...
                args=(config, queues["parakeet"], stop_event, emit),
                daemon=True,
            )
        )
    ...
    emit("system", f"Listening on device={config.device if config.device is not None else 'default'}")
    emit("system", f"Providers: {', '.join(config.providers)}")
    emit("system", "Press Ctrl+C to stop.")
...
def _run_deepgram_live(
    ...
    def on_message(message) -> None:
        ...
        if transcript:
            emit("deepgram*", transcript if not is_final else f"FINAL {transcript}")

    def on_error(error) -> None:
        emit("deepgram", f"error: {error}")
        stop_event.set()
...
def _run_voxtral_live(
    ...
    asyncio.run(_run_voxtral_live_async(config, audio_queue, stop_event, emit))
    ...
    try:
        async with websockets.connect(config.voxtral_uri, open_timeout=10.0, max_size=None) as ws:
            ...
            if hello.get("type") != "session.created":
                emit("voxtral", f"unexpected handshake: {hello}")
                stop_event.set()
                return

            emit("voxtral", "connected")
            ...
            async def recv_text() -> None:
                async for raw_message in ws:
                    ...
                    if message.get("type") == "transcription.delta":
                        delta = message.get("delta", "").strip()
                        if delta:
                            emit("voxtral", delta)
                    elif message.get("type") == "transcription.done":
                        text = message.get("text", "").strip()
                        if text:
                            emit("voxtral", f"FINAL {text}")
                    elif message.get("type") == "error":
                        emit("voxtral", f"error: {message.get('error')}")
                        stop_event.set()
                        return
...
def _run_parakeet_live(
    ...
    try:
        ...
        emit("parakeet", f"error: failed to start worker: {exc}")
        stop_event.set()
        return
    ...
    def read_worker_output() -> None:
        ...
        for raw_line in proc.stdout:
            ...
            if msg_type == "status":
                emit("parakeet", message.get("message", "status"))
                ...
                text = message.get("text", "").strip()
                if text:
                    emit("parakeet", f"FINAL {text}")
            elif msg_type == "error":
                emit("parakeet", f"error: {message.get('message', 'unknown worker error')}")
                stop_event.set()
                return
    ...
    try:
        ...
        while not stop_event.is_set():
            if proc.poll() is not None:
                emit("parakeet", f"error: worker exited with code {proc.returncode}")
                stop_event.set()
                break
            try:
                chunk = audio_queue.get(timeout=0.1)
